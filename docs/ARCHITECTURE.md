...
| `catalog` | 1 | Packages (price, duration, shot count, allowed template categories, link TTL) | |
...
| `devices` | 1 | Booth registration, heartbeat, health snapshot | |
...
│   │   ├── hotfolder.printer.ts     # default (DNP/HiTi hot-folder utility)
│   │   └── spooler.printer.ts       # fallback: pdf-to-printer via Windows spooler
...
└── renderer/                    # React wizard — screens 1–10 of PRD F2
    ├── screens/                     # attract, code-entry, template, capture, select, ...
...
- `PACKAGE` carries: price, session duration, shot count, allowed categories, prints included, link TTL days, retention days.
...
| Audience | Mechanism |
|---|---|
| Staff/Admin | Email+password → JWT (short-lived) + refresh; RBAC `ADMIN`/`STAFF`; admin-only: templates, packages, users, retention overrides |
...
3. Install PhotoMob Booth (electron-builder NSIS); configure device token + API URL on first run.
