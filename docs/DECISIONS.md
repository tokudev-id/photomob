...
## ADR-018: Potoku Box — second product line, self-service, reuses the Electron booth (2026-07-15)

**Context**: Business pivot: sell compact, movable, self-service photobooth boxes to operators (venues, event organizers, café owners). The customer walks up, pays by QRIS on the spot, shoots, and takes a printed strip — no staff, no booking. Toku's direction: start from the existing `potoku` Electron app rather than a new client.
**Decision**: The Box is a **second product line on the same platform**, not a fork. Same three repos, same API, same tenancy/gallery/enrollment. The booth app gains a server-owned **operating mode** per device: `staffed` (today's F2 code-entry flow) or `self-service` (F7 pay-to-start flow). Mode is business config and therefore server-owned (ADR-015 split unchanged); hardware picks stay booth-local. Box hardware target is a **small Windows touch device** (touch-screen tablet/mini-PC class) because Electron does not run on Android/iPadOS — this is the price of full code reuse and it buys us the already-built kiosk shell, webcam adapter, offline queues, updater, and ADR-017 enrollment on day one. Camera is the built-in/USB **webcam** (existing adapter); DSLR is not part of the Box. The premium staffed DSLR booth remains a supported product for event/store tiers.
**Trade-off**: Windows-class box hardware costs more per unit than an Android tablet. Accepted for time-to-revenue; the escape hatch is already enforced by the booth iron rule (renderer never touches hardware, everything crosses typed IPC), so porting the renderer to an Android/PWA shell later is a contained project, recorded as evolution — not designed now.

## ADR-019: In-box QRIS payment on the platform's Midtrans; operator earnings ledger (2026-07-15)

**Context**: The Box customer pays at the machine. Toku's model: operators **buy the box and pay a SaaS subscription**, while customer payments settle to **Toku's Midtrans account** — one merchant onboarding, platform controls the money flow, operators are paid out.
**Decision**: The box requests a session purchase; the API creates a walk-in **Booking in `PendingPayment`** (the same aggregate M5 defined — never a second money model) plus a Midtrans **Core API dynamic QRIS** charge, and returns the QR payload for on-screen display. The **webhook remains the only trusted "paid" path** (verify-then-trust re-query, M5 rule); the box polls the API for payment status — it never talks to Midtrans. On paid: booking confirms, the session is created and bound to the paying box directly (no session code — the box *is* the booth). Every paid session writes an **earnings ledger entry** per tenant/store: gross, gateway fee, platform fee, net. Payouts to operators are **manual bank transfer in v1** against an exportable settlement report; Iris/disbursement automation is a later upgrade. Unpaid QR expires with the charge (~15 min cap, config-driven); the attract loop resumes.
**Trade-off**: Platform-collected money makes Toku the merchant of record — refunds, disputes, and payout bookkeeping land on the platform, and the ledger must be correct from day one. Accepted: it removes merchant onboarding as a sales blocker, and the ledger doubles as the operator-facing revenue dashboard. Staff-recorded payments (ADR-005) and M5 online-booking payments are unchanged.

## ADR-020: Thermal strip is a first-class print target; photo printer is the upgrade tier (2026-07-15)

**Context**: A dye-sub printer adds Rp 4–8jt, bulk, and media logistics per box. A 58/80mm thermal receipt printer costs under Rp 1jt, prints for ~free, and the mono "receipt photo strip" is a novelty customers photograph and share. The real photos already live in the digital gallery (ADR-010).
**Decision**: The Box launches thermal-first. A new **thermal render target** in the booth (not in `@potoku/template-kit`'s color pipeline): 576px-wide (80mm) raster — dithered mono (Floyd–Steinberg) photo stack, brand header, gallery QR + PIN, footer copy — sent as ESC/POS raster (`GS v 0`) over USB or TCP:9100 behind the existing `IPrinter` seam as a new adapter. The full-color composed strip is still produced and uploaded unchanged — gallery quality is untouched; thermal is *an output*, not the compose. Print failure never blocks the QR screen (F2 rule holds). The dye-sub path built in M1 (DNP hot-folder / HiTi driver) becomes the Box's **photo upgrade tier** later — per-box printer config, zero new architecture. This supersedes the "later" status of ESC/POS in ADR-013 (that ADR concerned staff receipts; this one prints the product itself).
**Trade-off**: Mono thermal output is objectively low-fidelity; we are betting the QR-to-gallery handoff plus novelty factor carries it. If it doesn't, the upgrade tier is a config change and a certified printer, not a redesign.

## ADR-021: SaaS machinery activates — self-signup tenants, subscription enforcement, platform-admin surface (2026-07-15)

**Context**: ADR-012 deferred SaaS machinery "until a second client exists". Selling boxes creates that client base; every box operator is a tenant.
**Decision**: Activate the deferred half of ADR-012: public **tenant self-signup** (operator → tenant + first store + admin user, email-verified), a **subscription** per tenant (per-box/month plans, integer IDR) with lifecycle `Trial → Active → PastDue → Suspended`, and a **platform-admin** role above tenants (Toku operating the fleet: tenants, subscriptions, earnings, payouts, box health across everyone). Enforcement rule: an expiring subscription **never interrupts a paid customer session and never strands paid-but-unprinted media**; boxes enter "not in service" only from the attract screen after a grace window. Subscription billing v1 is **recorded, not gateway-automated** (invoice + platform-admin marks paid — same philosophy as ADR-005); Midtrans recurring is a later upgrade. Tenant data isolation, brand-as-data, and `storeId`/`tenantId` scoping already exist — this ADR adds machinery, not schema philosophy.
**Trade-off**: Manual billing means chasing payments by hand at small scale — acceptable while operator count is low and it keeps M10 shippable. Platform-admin is a new privilege tier that must be audited like staff/admin (API-026 pattern) from its first endpoint.
